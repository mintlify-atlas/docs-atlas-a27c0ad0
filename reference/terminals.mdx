...
Enemies are spawned by the main loop under two conditions:

- **Per-level cap** — up to `4 + level * 2` enemies exist at once (e.g. 10 in Level 4). Spawned at 2% probability per frame at a random position at least 300 px from the player.
- **Level 1 special** — an additional spawn condition allows up to 6 enemies at 8% probability, spawning at fully random screen positions (no distance check).
